...
from skillock.scanner import Finding, detect_skills, scan_tree
...
def test_destructive_rm_flag_orders(tmp_path):
    ...
    assert len(findings) == len(cases), (
        f"Expected {len(cases)} findings, got {len(findings)}: {[f.match for f in findings]}"
    )
...
def test_destructive_rm_false_negatives(tmp_path):
    ...
    assert len(findings) == 0, (
        f"Expected 0 findings, got {len(findings)}: {[f.match for f in findings]}"
    )
