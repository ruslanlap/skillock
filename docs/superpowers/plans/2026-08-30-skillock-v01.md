...
def cmd_add(args) -> int:
    ...
    entry = {
        "name": skill.name, "repo": url, "ref": tag or (sha if pinned == "commit" else git.tags(url)[0]),
        "pinned": pinned, "sha": sha,
...
def test_update_shows_diff_and_upgrades(home, tmp_path):
    ...
    assert run("add", str(repo), "--skill", "s", "--agents", "claude", "--yes", home=home).returncode == 0
